...
foreach ($scadenze as $i => $scadenza) {
    ...
    echo '
                            <tr class="'.$class.'">
                                ...
                                <td>
                                    <a onclick="launch_modal(\''.tr('Registra contabile pagamento').'\', \''.base_path_osm().'/add.php?id_module='.$id_modulo_prima_nota.'&id_scadenze='.$scadenza['id'].'&singola=1\');" class="btn btn-sm btn-primary">
...
'{[ "type": "select", "name": "id_banca_azienda[-id-]", "ajax-source": "banche", "select-options": '.json_encode(['id_anagrafica' => $anagrafica_azienda->id]).', "icon-after": "add|'.$id_modulo_banche.'|id_anagrafica='.$anagrafica_azienda->id.'", "onchange": "aggiornaScadenzaNuova(-id-)", "onblur": "aggiornaScadenzaNuova(-id-)" ]}'
                ...
                <td>
                    <a onclick="launch_modal(\''.tr('Registra contabile pagamento').'\', \''.base_path_osm().'/add.php?id_module='.$id_modulo_prima_nota.'&id_scadenze=-id-&singola=1\');" class="btn btn-sm btn-primary">
                        <i class="fa fa-euro"></i> '.($dir == 'entrata' ? tr('Incassa') : tr('Paga')).'
                    </a>
                </td>
            </input>
